...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(final HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers(HttpMethod.GET, "/api/init").permitAll()
				...
				.antMatchers(HttpMethod.PUT, "/api/post/{id}").authenticated()
				.antMatchers(HttpMethod.POST, "/api/comment/").authenticated()
				.antMatchers(HttpMethod.GET, "/api/tag/").permitAll()
				.antMatchers(HttpMethod.POST, "/api/moderation").hasRole("MODERATOR")
